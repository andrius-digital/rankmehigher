#!/usr/bin/env python3
"""Rogue Carrier Inc — OTR Company Driver recruiting reel.

PiP avatar bottom-right, canonical caption style. Generates 6 truck-themed
b-rolls, voice (Young Jamal), music (Fal stable-audio), HeyGen avatar, then
...
Usage:
    python3 rogue_reel.py --out-dir /opt/data/artifacts/rogue-company-driver
"""
...
BRAND_NAME = "Rogue Carrier Inc"
VOICE_ID = "6OzrBCQf8cjERkYgzSg8"  # Young Jamal
SPEED = 1.0

SSML = (
    'Seventy cents a mile plus a safety bonus. <break time="400ms"/>'
    'Or thirty percent of gross. <break time="300ms"/>'
    'Your choice. <break time="500ms"/>'
    'Rogue Carrier is hiring OTR company drivers right now. <break time="400ms"/>'
    'Thirty five hundred miles a week. <break time="300ms"/>'
    'All miles paid. Loaded and empty. <break time="400ms"/>'
    'Weekly direct deposit. <break time="300ms"/>'
    'Two thousand two hundred to three thousand a week. <break time="500ms"/>'
    'Automatics. APU. Fridge. Microwave. <break time="400ms"/>'
    'Freightliner. Volvo. Peterbilt. <break time="400ms"/>'
    'Fuel cards. Tolls paid. <break time="300ms"/>'
    'Three weeks out, three days home. <break time="400ms"/>'
    'Pets allowed. <break time="300ms"/>'
    'Two years CDLA. Clean record. No SAP. <break time="400ms"/>'
    'Stop scrolling and apply now. <break time="300ms"/>'
    'Link in bio.'
)

BROLL_PROMPTS = [
    # 0-5s — Hook: money/pay visual
    "Cinematic close-up of a trucker's hand counting a thick stack of hundred dollar bills on a truck dashboard, warm golden light through windshield, shallow depth of field, vertical 9:16, photorealistic",
    # 5-10s — Truck fleet: Freightliner/Peterbilt on highway
    "Cinematic wide aerial shot of a red and black Freightliner Cascadia semi truck with dry van trailer driving on an American interstate highway at golden hour, drone following shot, vertical 9:16, photorealistic",
    # 10-15s — Miles/open road
    "Cinematic POV shot from inside a semi truck cab looking through the windshield at a long straight American interstate highway stretching to the horizon, clear sky, speed lines, vertical 9:16, photorealistic",
    # 15-20s — Truck interior: modern amenities
    "Cinematic interior shot of a modern semi truck sleeper cab showing APU controls, mini fridge, microwave, clean organized space, warm ambient lighting, comfortable, vertical 9:16, photorealistic",
    # 20-25s — Home time: driver arriving home
    "Cinematic shot of a male trucker walking toward a suburban house front door carrying a duffel bag, warm afternoon sunlight, joyful anticipation, family home, vertical 9:16, photorealistic",
    # 25-30s — CTA: phone/apply
    "Cinematic close-up of a male trucker's hand holding a smartphone showing a job application screen inside a sunlit truck cab, warm golden hour light through windshield, focused, vertical 9:16, no readable text",
]

MUSIC_PROMPT = (
    "Dark aggressive trap beat, hard 808 bass, snappy snares, tight hi-hats, "
    "cinematic trucking recruitment ad, instrumental, no vocals, confident "
    "energy, 30 seconds"
)
...
CAPTION_DISPLAY_OVERRIDES: dict[str, str] = {
    ...
    "apu": "APU",
    "sap": "SAP",
    "roguecarrierinc.com": "ROGUECARRIERINC.COM",
    "seventy": "70",
    "thirty": "30%",
    "two": "2",
}
...
AVATAR_ID = os.environ.get(
    ...
    "4e1f282e54274850919da8f558a73190",
)
...
def composite(brolls: list[Path], voice: Path, music: Path, ass: Path,
              ...
              out: Path) -> Path:
    final = out / "rogue_reel_final.mp4"
...
def main() -> int:
    ...
    print(f"[caps] {len(words)} caption tokens", flush=True)
    ...
    if not a.no_telegram:
        telegram_send(final, caption=f"{BRAND_NAME} — OTR Company Driver recruiting reel. 9:16, AI-generated.")
